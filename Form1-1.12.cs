...
namespace SchemaLiteUI
{
	...
	///	1.00	2005-08-07	Markus		Base version
	/// </summary>
	public class Form1 : System.Windows.Forms.Form, Observer
	{
		...
		private void InitializeComponent()
		{
			...
			this.Day7.MouseMove += new System.Windows.Forms.MouseEventHandler(this.panel2_MouseMove);
			// 
			// Day6
			// 
			...
			this.Day6.MouseMove += new System.Windows.Forms.MouseEventHandler(this.panel2_MouseMove);
			// 
			// Day5
			// 
			...
			this.Day5.MouseMove += new System.Windows.Forms.MouseEventHandler(this.panel2_MouseMove);
			// 
			// Day2
			// 
			...
			this.Day2.MouseMove += new System.Windows.Forms.MouseEventHandler(this.panel2_MouseMove);
			// 
			// Day3
			// 
			...
			this.Day3.MouseMove += new System.Windows.Forms.MouseEventHandler(this.panel2_MouseMove);
			// 
			// Day4
			// 
			...
			this.Day4.MouseMove += new System.Windows.Forms.MouseEventHandler(this.panel2_MouseMove);
			// 
			// Day1
			// 
			...
			this.Day1.MouseMove += new System.Windows.Forms.MouseEventHandler(this.panel2_MouseMove);
			// 
			// Day8
			// 
			...
			this.Text = "SchemaLite Version 1.00";
		...
		public void updateWeekUI()
		{
			...
			// Hide marker
			SchemaUI.marker_.Size = new Size(0,0);
			this.infoLabel.Text = "";
			this.setDayTitles();
			// Create ActivityUIs
			...
			for ( int i=0; i < SchemaUI.DAYS_SHOWN; i++)
			{
				...
				if (Scheme.Instance.Activitys.ContainsKey(firstday.AddDays(i)) == true )
				{
					...
					acui = new ActivityUI(activity, true);
				}
				else
				{
					activity = new Activity(firstday.AddDays(i),"");
					acui = new ActivityUI(activity, false);
				}
				if ( i < 7 )
					panel1.Controls.Add(acui);
				else
					panel2.Controls.Add(acui);
				acui.BringToFront();
			...
			if ( Scheme.Instance.ResourceBoxPerWeek.ContainsKey(new SelectedWeek(Scheme.Instance.SelectedWeek.DateOfFirstDayInWeek)) )
			{
				foreach ( ResourceBox rb in ((SortedList)Scheme.Instance.ResourceBoxPerWeek[new SelectedWeek(Scheme.Instance.SelectedWeek.DateOfFirstDayInWeek)]).Values )
				{
					panel3.addResourcebox(rb);
				}
			}
			if ( Scheme.Instance.ResourceBoxPerWeek.ContainsKey(new SelectedWeek(Scheme.Instance.SelectedWeek.DateOfFirstDayInWeek.AddDays(7))) )
			{
				foreach ( ResourceBox rb in ((SortedList)Scheme.Instance.ResourceBoxPerWeek[new SelectedWeek(Scheme.Instance.SelectedWeek.DateOfFirstDayInWeek.AddDays(7))]).Values )
				{
					panel3.addResourcebox(rb);
				}
			}
...
//			panel3.DoDragDrop((String)checkedListBox.SelectedItem, DragDropEffects.Copy);
		...
		private void changeGrid()
		{
			...
			SchemaUI.SCHEME_DAY_PIXELS = (panel3.Width-SchemaUI.SCHEME_FIRST_DAY_PIXEL)/SchemaUI.DAYS_SHOWN;
		...
		private void Form1_SizeChanged(object sender, System.EventArgs e)
		{
			...
			this.panel3.Width = sideX-20-panel3.Left;
